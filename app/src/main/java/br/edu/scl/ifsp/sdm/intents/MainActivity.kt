...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        activityMainBinding.apply         {
            parameterBt.setOnClickListener {
                val parameterIntet = Intent( this@MainActivity,ParameterActivity::class.java).apply {
                    putExtra(PARAMETER_EXTRA, parameterTv.text)
                }
                parameterArl.launch(parameterIntet)
            }
        }
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            R.id.openActivityMi -> {
                val parameterActivity = Intent("OPEN_PARAMETER_ACTIVITY_ACTION").apply {
                    putExtra(PARAMETER_EXTRA, activityMainBinding.parameterTv.text)
                }
                parameterArl.launch(parameterActivity)
                true
            }
            R.id.viewMi -> {
            ...
            R.id.chooserMi -> {
                startActivity(
                    Intent(ACTION_CHOOSER).apply { putExtra(EXTRA_TITLE,
                        getString(R.string.choose_your_favorite_browser))
